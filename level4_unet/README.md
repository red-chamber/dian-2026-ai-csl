...
数据来源：`reports/metrics/unet_{l1,mse,l1grad}.json` 及其 `_test.json`。三组用完全相同的超参数与数据划分，只有 `--loss` 不同，因此差异可以归因到损失函数上。
...
- SSIM 的排序是 L1+梯度 > L1 > MSE，与预期完全一致。MSE 优化的条件均值会让笔迹边缘留下灰影，结构相似度吃亏，边缘保真度最好。
- PSNR 上 MSE 最高（24.86 dB），与预期一致。
