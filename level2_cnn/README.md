...
以下数字由 `python level2_cnn/src/train.py` + `python level2_cnn/src/compare.py` 实际跑出
（2026-09-22，commit `3862cd6`，seed 42），完整数据见 `reports/metrics/compare_mlp_cnn.json`。
...
| MLP | 98.16% | 184 |
| CNN | 99.06% | 94 |
| 提升 | +0.90 个百分点 | 少错 90 张 |

CNN 用 22.3% 的参数量把测试准确率抬到 99.06%。
这里 MLP 的 98.16% 与 Level 1 记录的数字完全一致，说明两者的数据划分、随机种子、
评估口径都没有变动，两组数字放在一起比是有意义的。
...
| 达到 96% 所需 epoch | 第 2 轮 | 第 1 轮 |
| 每轮平均耗时 | 3.8 s | 3.6 s |
| 最优 epoch / 验证准确率 | 第 7 轮 / 97.98% | 第 7 轮 / 98.85% |

CNN 第 1 轮就越过 96%，MLP 要到第 2 轮。
值得注意的是每轮耗时几乎相同（3.6 s vs 3.8 s）：CNN 参数量只有 MLP 的五分之一，
速度却没有优势，因为 MNIST 的规模太小，两种模型都跑不满 GPU，瓶颈不在计算量上。
...
| 类别 | 张数 | 说明 |
|---|---|---|
| 两者都错 | 48 | 两个模型都搞不定的硬样本 |
| 只有 MLP 错 | 136 | **CNN 修好了的样本，是卷积带来收益的直接证据** |
| 只有 CNN 错 | 46 | MLP 对而 CNN 错的样本，用来审视 CNN 的短板 |

48 + 136 = 184，正是 MLP 的全部错误数；48 + 46 = 94，正是 CNN 的全部错误数。

「只有 MLP 错」136 张，约是「只有 CNN 错」46 张的 3 倍。这说明提升不是简单的此消彼长，
而是 CNN 净多救回了 136 − 46 = 90 张图。错误样本网格见 `reports/samples/mlp_vs_cnn_errors.png`。

另外统计各自最容易混淆的数字对（`真实 X 被认成 Y`）：
...
两边的头号混淆都落在 7/9、4/9 这类书写时容易连笔或断笔的数字对上。
